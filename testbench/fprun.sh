...
sed -i "80s/localparam.*/localparam IDIV_ON_FPU = 32\'d1;/" $WALLY/config/rv64gc/config.vh
}
setIDIV_OFF(){

sed -i "80s/localparam.*/localparam IDIV_ON_FPU = 32\'d0;/" $WALLY/config/rv64gc/config.vh
}
...
gen2(){
    mkdir $WALLY/testbench/runs_noidiv
    cd ../sim
    setQ_ON
    setD_ON
    setK1
